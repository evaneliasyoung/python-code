...
ARGS.path = ARGS.path[:-1] if ARGS.path.endswith('/') or ARGS.path.endswith(
    '\\') else ARGS.path
    ...
    for i in os.listdir(path):
        ...
        if os.path.isdir(item_path):
            ...
            attrs = {
                ...
                i,
                "permissions":
                oct(os.stat(os.path.join(path, i)).st_mode & 0o0777)[-3:]
            }
            dir_trace = 0
            if i.startswith('.'):
                attrs['hidden'] = '1'
...
if ARGS.o:
    ...
    cmd: str = f'{stpre} '
    cmd += os.path.join(os.path.dirname(os.path.realpath(__file__)), os.path.basename(ARGS.path))
    cmd += '.xml'
    os.system(cmd)
